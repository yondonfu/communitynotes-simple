# Source: https://github.com/ethereum/research/blob/master/community_notes_analysis/basic_algo.py
...
def cost(matrix, intercept, user_friendliness, note_quality, user_alignment, note_alignment):
    ...
    total += 0.15 * \
        sum(v**2 for v in ([intercept] * size +
            user_friendliness + note_quality))
...
def naive_descent(matrix):
    ...
    print("Group 2 quality: {}".format(
        average(note_quality[size//4: size//2])))
    print("Group 3 quality: {}".format(
        average(note_quality[size//2: -size//4])))
        ...
        for j in range(size):
            if shared_l[i, j] > 0:
                for note in range(size):
                    note_quality[note] += adj_m_l[i, note] * \
                        adj_m_l[j, note] / shared_l[i, j]
            if shared_d[i, j] > 0:
                for note in range(size):
                    note_quality[note] -= adj_m_d[i, note] * \
                        adj_m_d[j, note] / shared_d[i, j]
    print("Group 1 quality: {}".format(average(note_quality[:size//4])))
    print("Group 2 quality: {}".format(
        average(note_quality[size//4: size//2])))
    print("Group 3 quality: {}".format(
